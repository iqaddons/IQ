...
object IqNanoVg {
    ...
    fun resetAfterRenderTransition(reason: String) {
        IqNanoVgConfiguration.mode().runReset {
            ...
            if (runtime == null) {
                logger.info("IQ NanoVG reset requested after {}, but no NanoVG runtime/context was created; delaying render for {} frames", reason, RECREATE_DELAY_FRAMES)
                return@runReset
            }
    ...
    fun renderCurrentScreen(client: Minecraft) {
        IqNanoVgConfiguration.mode().runScreen {
            val screen = client.screen as? IqNanoVgRenderable ?: return@runScreen
