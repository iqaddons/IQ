...
object IqHudNanoRenderer {
    ...
    fun renderCurrentHud(client: Minecraft) {
        ...
        if (client.player == null || client.level == null || client.gui.hud.isHidden() || client.options.keyPlayerList.isDown) return
    ...
    private fun drawEditorSelection(
        ...
        if (Minecraft.getInstance().gui.screen() !is HudEditScreen) return
        val rect = Rect(
    ...
    private fun drawCratePriority(renderer: Renderer2D, widget: CratePriorityWidget, mouseX: Double, mouseY: Double, backdropMode: Boolean) {
        val preview = Minecraft.getInstance().gui.screen() is HudEditScreen
